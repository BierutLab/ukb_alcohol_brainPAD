...
10/14/24 Added names for the PC 25 rfMRI IDPs.

6/14/23 Double check naming of global measures derived from ASEG, label ASEG mean intensity in the dimension column. I removed the cerebellum measures created by adding up FAST subcortical regions as we are now using cerebellum volume from Freesurfer ASEG.

9/2/22 Added 8 "IDP"s representing whole cerebellum volume, left cerebellum, right cerebellum, cerebellar vermis, anterior lobe, superior posterior lobe, inferior lobe, and flocculonodular lobe

8/8/22 Added a field for cortical / subcortical / cerebellar / other
...
```{r}
# Load IDP names
IDP_names <- read.delim(file = paste0(input_path, "IDPs_37741_names.txt"), sep = "#", header = FALSE)
...
Create the rfMRI independent component names (these are derived from IDPs)
```{r}
...
Stick the rfMRI independent component names onto the main table
```{r}
...
Add lines for the P25 partial correlation rfMRI - we use these in some versions of the brain age model
```{r}

P25_rfMRI<- data.frame("numeric_IDP" = seq(1, 210))

P25_rfMRI <- P25_rfMRI %>%
  mutate(IDP_name = paste0("Partial_corr_25_dim_25752_2_0_v", numeric_IDP)) %>%
  mutate(IDP = paste0("P25_", numeric_IDP)) %>%
  relocate(numeric_IDP, .after = IDP) %>%
  mutate(MRI = "rfMRI") %>%
  mutate(map = "P25") %>%
  mutate(dMRI = NA) %>%
  mutate(dimension = NA) %>%
...
Stick onto the main table
```{r}

IDP_names <- rbind(IDP_names, P25_rfMRI)
